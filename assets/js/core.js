...
const store = {
    ...
    save: () => {
        ...
        if (store.currentUser && window.fireMethods && window.fireDb) {
            // ✅ CORREÇÃO: Usando doc e setDoc (Firestore) ao invés de ref e set
            const { doc, setDoc } = window.fireMethods;
            
            // Define o caminho: Coleção 'users', Documento = UID do usuário
            const userDocRef = doc(window.fireDb, 'users', store.currentUser.uid);
            
            // setDoc com { merge: true } é mais seguro pois não apaga campos extras se houverem
            setDoc(userDocRef, dataToSave, { merge: true })
                .then(() => console.log("☁️ Sincronizado com sucesso (Firestore)"))
    ...
    updateAudioLink: (id, link) => {
        const r = store.reviews.find(i => i.id === id);
        if(r) {
            if(r.batchId) {
                // Propaga o áudio para todo o ciclo (revisões futuras)
                store.reviews.forEach(item => { if(item.batchId === r.batchId) item.audioLink = link; });
            } else { 
                r.audioLink = link; 
            }
            store.save();
            if (typeof ui !== 'undefined') ui.render();
            toast.show('Áudio do NotebookLM anexado.', 'success');
        }
    }
};
