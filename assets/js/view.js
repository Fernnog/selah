 ...
 * Versão: 2.0.0 - Integração "Selah x CicloSmart"
 */
...
const ui = {
     ...
     * Refatorado para suportar fileManager e estilos do CicloSmart.
     */
    createCardHTML: (review) => {
        const isDone = review.status === 'DONE';
        
        // Estilização Base (Stone/Amber)
        ...
        // 2. Anexo HTML (Lógica herdada do CicloSmart)
        const hasSummary = review.htmlSummary && review.htmlSummary.length > 0;
        const summaryBtn = `
            <button onclick="fileManager.handleAction('${review.id}', ${hasSummary}); event.stopPropagation();"
                    class="w-7 h-7 flex items-center justify-center rounded-full border transition-all ${hasSummary ? 'bg-emerald-50 text-emerald-600 border-emerald-200 hover:bg-emerald-100' : 'bg-stone-50 text-stone-300 border-transparent hover:text-emerald-600 hover:bg-emerald-50'}" 
                    title="${hasSummary ? 'Ler Reflexão' : 'Anexar HTML'}">
                <i data-lucide="${hasSummary ? 'file-text' : 'upload-cloud'}" class="w-3.5 h-3.5"></i>
            </button>`;
