 ...
 * Versão: 1.2.0 - Implementação de Contadores e Áudio NotebookLM
 */
...
const ui = {
    ...
    createCardHTML: (review) => {
        ...
        // LÓGICA DO NOVO BOTÃO DE ÁUDIO (NotebookLM)
        const hasAudio = review.audioLink && review.audioLink.length > 5;
        const audioBtn = `
            <button onclick="app.handleAudioAction('${review.id}', '${review.audioLink || ''}')"
                    class="w-7 h-7 flex items-center justify-center rounded-full border transition-all ${hasAudio ? 'bg-violet-50 text-violet-600 border-violet-200 hover:bg-violet-100' : 'bg-stone-50 text-stone-300 border-transparent hover:border-stone-200'}" 
                    title="${hasAudio ? 'Ouvir Resumo (NotebookLM)' : 'Adicionar Áudio (NotebookLM)'}">
                <i data-lucide="${hasAudio ? 'headphones' : 'mic'}" class="w-3.5 h-3.5"></i>
            </button>`;
    ...
    render: () => {
        ...
        ['late', 'today', 'future'].forEach(k => {
            // Contadores do Header Desktop
            const el = document.getElementById(`count-${k}`);
            if(el) el.innerText = counts[k];

            // Novos Contadores Redondos das Abas Mobile
            const badge = document.getElementById(`badge-${k}`);
            if (badge) {
                badge.innerText = counts[k];
                if (counts[k] > 0) {
